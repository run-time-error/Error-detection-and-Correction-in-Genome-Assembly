...
int lengths[noContigs+10];
main()
{
    ifstream cLen("ContigLength");
...
//    cout << lengths[314] << endl;
    ...
    while(!inFile.eof())
    {
        ...
        ofstream outFile("ProbFolder1/"+fileName+".txt");
        ...
        {
            cout<<curContig<<"\t"<<lengths[curContig]<<endl;

            /// loop for each contig c
            double probability = 0.0;
            ofstream probf("probtest.txt");
            queue<qE> Q;
            for(int i=0;i<lengths[curContig];i++){
